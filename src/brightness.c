...
void apply_bright_range_lines(float brilho, imagem *I, int start, int end) {
	int index;
	float pixval = 0;
...
void bright_dispatcher(float brilho, imagem *I, int jobs, pid_t *processes) {
	...
	for (index = 0; index < jobs; index++) {
		...
		if (processes[index] == 0) {
			...
			apply_bright_range_lines(brilho, I, start, end);
			exit(0);
		}
	}
}
...
void apply_bright_single_thread_lines(float brilho, imagem *I) {
	...
	start = clock();
	apply_bright_range_lines(brilho, I, 0, imgsize);
	end = clock();
...
void apply_bright_single_thread_columns(float brilho, imagem *I) {
	...
	start = clock();
	apply_bright_range_columns(brilho, I, 0, imgsize);
	end = clock();
	
	dbgmsg("Image processing strategy: single thread, operating by columns.\n");
	dbgmsg("Elapsed time: %f seconds.\n", (double)(end - start)/CLOCKS_PER_SEC);
}
